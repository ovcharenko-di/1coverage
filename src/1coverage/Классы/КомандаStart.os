...
Процедура СоздатьФайлPID()

	ПутьКФайлуPID = ОбъединитьПути("build", "PID");
	Текст = Новый ТекстовыйДокумент();
	Текст.Записать(ПутьКФайлуPID);
...
Процедура ЗапуститьПрокси()
	...
	КаталогПроекта = ПараметрыПриложения.КаталогПроекта;
	КаталогПроектаВКавычках = КонсольнаяКоманда.ОбернутьВКавычки(КаталогПроекта);
	
	ПолныйПутьКЛогамПрокси = ОбъединитьПути(ТекущийКаталог(), ПутьКЛогамПрокси);
	ПолныйПутьКЛогамПроксиВКавычках = КонсольнаяКоманда.ОбернутьВКавычки(ПолныйПутьКЛогамПрокси);
	
	КонсольнаяКоманда.УстановитьКоманду("npm");
	КонсольнаяКоманда.ДобавитьПараметр("--prefix");
	КонсольнаяКоманда.ДобавитьПараметр(КаталогПроектаВКавычках);
	КонсольнаяКоманда.ДобавитьПараметр("start");
	КонсольнаяКоманда.ДобавитьПараметр("--");
	КонсольнаяКоманда.ДобавитьПараметр("--logpath");
	КонсольнаяКоманда.ДобавитьПараметр(ПолныйПутьКЛогамПроксиВКавычках);
