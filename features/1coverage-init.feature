...
Сценарий: Запуск приложения с командой init для существующей файловой базы внутри произвольного каталога
    ...
    И Я выполняю команду "oscript" с параметрами "<КаталогПроекта>/src/1coverage/1coverage.os -v init --ibconnection /F./build/ib"
...
Сценарий: Запуск приложения с командой init для существующей файловой базы с указанием порта
    Когда Я создаю временный каталог и сохраняю его в контекст
    И Я устанавливаю временный каталог как рабочий каталог
    И Я установил рабочий каталог как текущий каталог
    Когда Я выполняю команду "runner" с параметрами "init-dev --root <РабочийКаталог>"
    И Я выполняю команду "oscript" с параметрами "<КаталогПроекта>/src/1coverage/1coverage.os -v init --ibconnection /F./build/ib --dbgs-proxy-port 5555"
    И Я сообщаю вывод команды "oscript"
    Тогда Вывод команды "oscript" содержит "Установлена http-отладка для базы"
    И код возврата равен 0
    И я устанавливаю каталог настроек ИБ "/F./build/ib" как новый рабочий каталог
    И файл "<РабочийКаталог>/1cv8.pfl" содержит
    """
    {"S","localhost:5555"},"remoteDebugServerHistory",
    {"S",":5555"},"remoteDebugUseSpecifiedInfobaseAlias",
    """

Сценарий: Запуск приложения с командой init для существующей файловой базы внутри произвольного каталога без добавления в список
    Когда Я создаю временный каталог и сохраняю его в контекст
    И Я устанавливаю временный каталог как рабочий каталог
    И Я установил рабочий каталог как текущий каталог
    Когда Я выполняю команду "runner" с параметрами "init-dev --root <РабочийКаталог> --nocacheuse"
    И Я выполняю команду "oscript" с параметрами "<КаталогПроекта>/src/1coverage/1coverage.os -v init --ibconnection /F./build/ib"
    И Я сообщаю вывод команды "oscript"
    Тогда Вывод команды "oscript" содержит "Установлена http-отладка для базы"
    И код возврата равен 0
